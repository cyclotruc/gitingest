...
from core.repository_clone import CloneConfig, _check_repo_exists, clone_repo
...
async def test_clone_repo_with_commit() -> None:
    ...
    with patch("core.repository_clone._check_repo_exists", return_value=True) as mock_check:
        with patch("core.repository_clone._run_git_command", new_callable=AsyncMock) as mock_exec:
...
async def test_clone_repo_nonexistent_repository() -> None:
    ...
    with patch("core.repository_clone._check_repo_exists", return_value=False) as mock_check:
...
async def test_clone_repo_with_custom_branch() -> None:
    ...
    with patch("core.repository_clone._check_repo_exists", return_value=True):
        with patch("core.repository_clone._run_git_command", new_callable=AsyncMock) as mock_exec:
            await clone_repo(clone_config)
...
async def test_git_command_failure() -> None:
    ...
    with patch("core.repository_clone._check_repo_exists", return_value=True):
        with patch("core.repository_clone._run_git_command", side_effect=RuntimeError("Git command failed")):
