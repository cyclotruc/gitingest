...
async def test_parse_url_with_subpaths() -> None:
    ...
    with patch("CodeIngest.utils.git_utils.run_command", new_callable=AsyncMock) as mock_run_command:
        ...
        with patch(
            "CodeIngest.utils.git_utils.fetch_remote_branch_list", new_callable=AsyncMock
...
async def test_parse_repo_source_with_failed_git_command(url, expected_branch, expected_subpath):
    ...
    """
    with patch("CodeIngest.utils.git_utils.fetch_remote_branch_list", new_callable=AsyncMock) as mock_fetch_branches:
